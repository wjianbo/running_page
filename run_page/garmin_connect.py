...
class Garmin:
    ...
    async def get_strength_activities(self, start, limit):
        """
        Fetch available activities
        """
        url = f"{self.modern_url}/activitylist-service/activities/search/activities?start={start}&limit={limit}"
        url += f"&activityType={GarminActivityType.STRENGTH}"
        return await self.fetch_data(url)
...
async def get_activity_id_list(client, start=0):
    activities = await client.get_strength_activities(start, 100)
    if len(activities) > 0:
